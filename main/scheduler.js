...
Scheduler.DATE_FUNCTIONS = ["getUTCMinutes", "getUTCHours", "getUTCDate", "getUTCMonth", "getUTCFullYear"];
...
var triggerNow = function(arr, now) {
    for (var i = 0; i < 5; i++) {
        ...
        var val = now[Scheduler.DATE_FUNCTIONS[4-i]]();
        if (arr[i].indexOf("*/") == 0 && (val % parseInt(arr[i].substring(2), 10)) != 0) {
            return false;
        }
        if (val != parseInt(arr[i], 10)) {
            return false;
        }
    }
    return true;
...
var run = function(instance) {
    ...
    timeoutId = setTimeout(run, calcDelay(), instance);
};
