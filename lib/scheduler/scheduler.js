...
var triggerNow = function(arr, now) {
    for (var i = 0; i < 6; i++) {
        ...
        if (Array.isArray(arr[i])) {
            ...
            return false;
        } else if (arr[i].indexOf("*/") == 0 && (val % parseInt(arr[i].substring(2), 10)) != 0) {
            // value divided with the value after the slash has to have a remainder of 0
            return false;
        }
        log.warn("should never reach");
    }
    return true;
};

var checkScheduleElement = function(el) {
    if (!isNaN(el)) {
        return true;
    }
    if (el == "*") {
    ...
    if (el.indexOf("*/") == 0) {
        return !isNaN(el.substring(2)) && el.substring(2) != "0"; // avoid division by zero
    }
    return false;
};
